...
class Prog(Frame):
    ...
    def __init__(self, parent=None):
        ...
        if(enable_saslock == 'YES' or enable_saslock == 'yes'):
            if(os.path.isfile('.saslock')):
                ...
                ans = Message(title=message_warning, message=title + txt,
                              default='cancel', icon=QUESTION, type='okcancel').show()
	...
	"""
        ...
        if(desired_font_sizes['default_font'] != 'YES' and desired_font_sizes['default_font'] != 'yes'):
            ...
            root.customFont.configure(size=9)

            button_font = root.customFont
            local_font = root.customFont
            #button_font = ('Helvetica', 12,'bold')
            local_font = ('Helvetica', 9, 'bold')
            button_pady = 8

        self.option_add('*Font', local_font)

        vers = 'version 0.99_rev_1155 : 12/08/13'
        Label(self, text='---------------', fg='black', bg='white',
              font=local_font).pack(side=TOP, fill=BOTH)
        Label(self, text='SASSIE', fg='black', bg='white', font=local_font).pack(
            side=TOP, fill=BOTH)
        Label(self, text=vers, fg='black', bg='white', font=local_font).pack(
            side=TOP, fill=BOTH)
        Label(self, text='---------------', fg='black', bg='white',
              font=local_font).pack(side=TOP, fill=BOTH)
        ...
        for i in range(numitems):
            ...
            if(aprogs[i][1] != 'section_header'):
                ...
                if(desired_modules[lkey] != 'YES' and desired_modules[lkey] != 'yes'):
                    ...
                    this_button = Button(self, text=lkey, command=lvalue, font=button_font, relief=RIDGE, padx=8, pady=button_pady, bg='lightgrey',
                                         foreground='black', takefocus='', activeforeground='black', border=6, highlightbackground='lightgrey').pack(side=TOP, fill=BOTH, padx=10, pady=2)
                ...
                Label(
                    self, text=aprogs[i][0], bg='white', fg='#0198E1', font=local_font).pack()

        Label(self, text=' ', fg='black', bg='white').pack()
        Button(self, text="Quit", command=self.quit, font=button_font, relief=RIDGE, padx=12,
               pady=button_pady, bg='white', fg='red', border=4).pack(side=TOP, fill=BOTH, padx=10)
        ttxt = time.ctime()
        Label(self, text=ttxt, fg='black', bg='white', font=local_font).pack(
