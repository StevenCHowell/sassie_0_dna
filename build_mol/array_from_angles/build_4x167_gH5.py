...
def combine_sasmols(all_mols, combine_segnames=None):
    ...
    for (i, mol) in enumerate(all_mols):
        ...
        if i == 0:
            ...
            combined_mol.setOccupancy(
                combined_mol.occupancy() + mol.occupancy())
    ...
    if combine_segnames:
        for segname in combine_segnames:
            ...
            for j in xrange(i + 1):
                j_str = str(j + 1)
                tmp_segname = [
                    name.replace(j_str + segname, segname) for name in tmp_segname]

            combined_mol.setSegnames(tmp_segnames)
            error, mask = combined_mol.get_subset_mask(
                "segname[i] = '%s'" % segname)
...
def align_gH5_to_c11():
    ...
    try:
        ...
        for i in xrange(len(c11_filter)):
            ...
            assert s_link == s_c11, "filter for c11 has %d atoms but H5 has %d atoms" % (
                s_c11, s_link)
    ...
    all_ncp_plot_vars, all_ncp_axes, all_ncp_origins = ta.get_tetramer_axes(
        ...
        array=ncp_array)
    phi, dxyz, plot_title = ta.get_tetramer_angles(
        all_ncp_axes, all_ncp_origins)
    ...
    if 'slide' in method:
        ...
        for i in xrange(len(adjust_ncp.mv_ncp)):
            ...
            stack_axis = stack_axis / \
                np.sqrt(stack_axis.dot(stack_axis))  # unit v
            displacement = mag * stack_axis
            ncp_list[mv_ncp[0]].setCoor(
                ncp_list[mv_ncp[0]].coor() - displacement)
            ncp_list[mv_ncp[1]].setCoor(
                ncp_list[mv_ncp[1]].coor() + displacement)
    ...
    if 'rotate_sym' in method:
        ...
        for (i, mv_ncp) in enumerate(adjust_ncp.mv_ncp):
            ...
            for i in xrange(2):
                ...
                coor = ncp.coor()[0]
                all_coor = np.concatenate(
                    (center.reshape(1, 3), axes_coor, coor))

                # perform the rotation
                all_coor = geometry.rotate_about_v(
                    all_coor, stack_axis, angles[i])
