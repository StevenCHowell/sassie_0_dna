...
def align(variables,txtOutput):
	...
	runname,path,infile,pdbmol1,pdbmol2,basis1,lowres1,highres1,basis2,lowres2,highres2,ofile=unpack_variables(variables)
	...
	basis_filter_1 = 'name[i] == "'+basis1+'" and (resid[i] >= '+str(lowres1)+' and resid[i] <= '+str(highres1)+')'
	basis_filter_2 = 'name[i] == "'+basis2+'" and (resid[i] >= '+str(lowres2)+' and resid[i] <= '+str(highres2)+')'
	...
	for i in xrange(nf2):
		...
		m2.align(i,coor_sub_m2,com_sub_m2,coor_sub_m1,com_sub_m1)
                ...
                if(((i+1)%(float(nf2)/10.0)==0 or (nf2<10))):
                        ...
                        progress_string='COMPLETED '+str(i+1)+' of '+str(nf2)+' : '+str(fraction_done*100.0)+' % done'
                        print('%s\n' % progress_string)
	...
	min_x = total_min_array[0] ; max_x = total_max_array[0]
	min_y = total_min_array[1] ; max_y = total_max_array[1]
	min_z = total_min_array[2] ; max_z = total_max_array[2]
