...
'''
        ...
        that are compared using the modules in FILTER.
...
#
    ...
    for i in xrange(nsegments):
        ...
        name_filter = ''
        for j in xrange(len(allsbasis[i])):
            if j > 0:
                name_filter += ' or '
    ...
    for i in xrange(nsegments):
        ...
        for j in range(anregions[i]):
            seg_filter = ' segname[i] == "%s" ' % allsname[i]
            name_filter = ''
            for k in xrange(len(allsbasis[i])):
                if k > 0:
                    name_filter += ' or '
                name_filter += 'name[i] == "%s"' % allsbasis[i][k]
            name_filter = '( %s )' % name_filter
            resid_filter = ('( resid[i] >= %d and resid[i] <= %d )' %
                            (alow[i][j], ahigh[i][j]))
            basis_filter_2 = '( %s and %s and %s )' % (seg_filter, name_filter,
                                                       resid_filter)
