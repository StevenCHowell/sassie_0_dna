...
#        self.data[idx] /= (dflat.data[idx]/np.average(dflat.data[idx]))
    ...
    def avg(self, dsets, plot_data=False, ax=None, db=None):
        ...
        print 'max error (original, 1%): \n', np.argmax([self.err, self.data*.01], axis=0) # [0. 1.]
        self.err = np.max([self.err, self.data*.01], axis=0) # [0. 1.]
        for d1 in dsets:
            d1.err = np.max([self.err, self.data*.01], axis=0) # [0. 1.]
        ...
        n_exposures = len(dsets) + 1
        nq = len(self.data)
        assert nq == len(self.qgrid), 'bug here :('
        iq_exposures = np.zeros((n_exposures, nq))
        iq_exposures[0, :] = self.data
        for (i, d1) in enumerate(dsets):
            print "%s " % d1.label,
        ...
        self.trans /= n
        self.data  /= n
        self.err   /= n #s modified to be adding in quadrature
        #s self.err /= np.sqrt(n)
        '''
        set the error to the max of the following:
        ...
        print 'max error (pixel, exposures): \n', np.argmax([self.err, err_exposures], axis=0) # [0. 1.]
        self.err = err
    ...
    def bkg_cor(self, dbak, sc_factor=1., plot_data=False, ax=None):
        ...
        #s modified to add the error in quadrature
...
def avg_SWAXS(fns,sdark,sext="_SAXS",wdark=None,wext="_WAXS",qmax=-1,qmin=-1,reft=-1,
              plot_data=False,save1d=False,saxsflat=None,waxsflat=None,fix_scale=-1,
              ax=None,db=None):
    """
    fns: filenames, without the _SAXS/_WAXS suffix
