...
def make_cgDNA_model(all_atom_pdb, chain1, chain2, resid1, resid2, flexResid, bp_perBead):
    ...
    for j in xrange(nbeads):
        if j + 1 == nbeads:
            # remain = bps%bp_perBead # not sure if this is the best way to
        ...
        basis_filter = "((resid[i] > " + str(r1a - 1) + " and resid[i] < " + str(r1b) + ") and (chain[i]=='" + chain1 + \
            "')) or ((resid[i] > " + str(r2a - 1) + " and resid[i] < " + \
            str(r2b) + ") and (chain[i]=='" + chain2 + "'))"
        ...
        # store the coordinates of the com for the bead coordinate
        cg_coor[0, j, :] = com
        # print 'com ', j, '= ', com

        # calculate the coodinates of the atoms in the bead with reference to
        # the bead com
        bead.center
...
def beadRotate(coor3, vecX, vecY, vecZ, thetas, nSoft):
    ...
    # make sure the number of cg beads and orientation vectors match (this
    # will not include any rigid components)
    assert (
        natoms, col) == vecX.shape == vecY.shape == vecZ.shape, "different number of bead origins and orientations"
    ...
    # ; print 'step 0 rotated about first angle coor:\n', coor4
...
def dna_mc(nsteps, cg_dna, vecXYZ, lp, w, theta_max, flexible, nSoft=3, f=True):
    ...
    for i in xrange(nsteps):
        ...
        trial_bead = flexible(
            int((nflex - 1) * random.random()) + 1)  # CHECK THIS ##
        ...
        # s print 'coor before:\n',coor
        (coor[trial_bead:], vecX[trial_bead:], vecY[trial_bead:], vecZ[trial_bead:]) = beadRotate(coor[
            trial_bead - 1:], vecX[trial_bead - 1:], vecY[trial_bead - 1:], vecZ[trial_bead - 1:], thetaXYZ, nSoft)
        # s print 'coor after\n',coor
...
if __name__ == "__main__":
    ...
    for i in xrange(iters):
        (cg_dna, vecXYZ, a, r) = dna_mc(
            nsteps, cg_dna, vecXYZ, lp, w, theta_max, flexible, nSoft, f)
